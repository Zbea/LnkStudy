...
public class PaperDaoManager {
    ...
    public long getInsertId(){
        List<Paper> queryList = dao.queryBuilder().build().list();
        return queryList.get(queryList.size()-1).id;
    }

    public Paper queryByID(Long id) {
        Paper item = dao.queryBuilder().where(PaperDao.Properties.Id.eq(id)).build().unique();
        return item;
    }
     ...
     * @return
     */
    public List<Paper> queryAll(int userId,int type,int courseId, int categoryId) {
        WhereCondition whereCondition= PaperDao.Properties.UserId.eq(userId);
        ...
        List<Paper> queryList = dao.queryBuilder().where(whereCondition,whereCondition1,whereCondition2,whereCondition3).build().list();
        return queryList;
    }
