...
class CloudDownloadListDialog(val context: Context,val type:Int) {

    fun builder(): CloudDownloadListDialog {
        ...
        val items=ItemTypeDaoManager.getInstance().queryAllOrderDesc(type)

        val rv_list=dialog.findViewById<RecyclerView>(R.id.rv_list)
        rv_list?.layoutManager = LinearLayoutManager(context)
        val mAdapter = MyAdapter(R.layout.item_cloud_download, items)
        rv_list?.adapter = mAdapter
        mAdapter.bindToRecyclerView(rv_list)
        mAdapter.setEmptyView(R.layout.common_empty)
        mAdapter.setOnItemClickListener { adapter, view, position ->
            listener?.onClick(items[position].typeId)
            dialog.dismiss()
        }
                    ...
                    .setDialogClickListener(object : CommonDialog.OnDialogClickListener {
                        ...
                        override fun ok() {
                            when(type){
                                ...
                                3->{
                                    MethodManager.deletePaintingDrawing(0,item)
                                }
                                4->{
                                    MethodManager.deletePaintingDrawing(1,item)
                                }
                            }
