 ...
 * type=0壁纸 type=1书画
 */
class AppWallpaperListAdapter(layoutResId: Int, data: List<PaintingList.ListBean>?) : BaseQuickAdapter<PaintingList.ListBean, BaseViewHolder>(layoutResId, data) {

    override fun convert(helper: BaseViewHolder, item: PaintingList.ListBean) {
        helper.setText(R.id.tv_name,item.drawName)
        helper.setText(R.id.tv_price,if (item.price==0)"免费" else "${item.price}学豆")
